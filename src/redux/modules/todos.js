  ...
  const todos = (state = initialState, action) => {
    switch (action.type) {
      case NEW_TODO:
        ...
        case DELETE_TODO:
          const itemDeleted = state.filter((item)=>item.id !== action.payload.id)
          alert("deleted!")
          return state = itemDeleted


          case DONE_TODO:
            const doneTodo = state.map((item)=>item.id === action.payload.id?{...item, isDone : !item.isDone}:item)
          return state = doneTodo

        case EDIT_TODO:
          const startEdit = state.map((item)=>item.id === action.payload.id?{...item, isEdit : !item.isEdit}:item)
        return state = startEdit


        case UPDATE_TODO:
          const todoId = action.payload.id
          const targetIndex = state.findIndex((item)=> item.id === todoId )
          const newTodos = [...state]
          newTodos.splice(targetIndex, 1, {id:todoId, title:action.updates.editTitle, desc:action.updates.editDesc, isDone:action.payload.isDone, isEdit:false})
          alert("Todo edited!")
